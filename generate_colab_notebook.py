...
==========================
Generates the updated, production-ready Google Colab Jupyter Notebooks:
  - kabadiwala_colab_master_pipeline.ipynb
  - sih_kabadiwala_ver1.ipynb
"""
    ...
    cells = [
        {
            ...
            "metadata": {},
            "source": [
                "# ♻️ Kabadiwala — 16-Class Zero-Bias Scrap Vision Training Pipeline\n",
                "**TrashBox-testandvalid + TrashBox + TrashNet | Early Stopping | MobileNetV3 & DINOv2**\n",
                "\n",
                "This master notebook downloads open scrap datasets, eliminates majority-class bias through balanced sampling (~500 images per class), applies Class-Weighted Cross-Entropy Loss, and trains fast and accurate models with **Early Stopping** based on validation Macro F1 score.\n",
                "\n",
                "### Table of Contents\n",
                "- **01** — Environment Setup & Drive Mount\n",
                "- **02** — Dataset Ingestion (TrashBox-testandvalid, TrashBox, TrashNet)\n",
                "- **03** — 16-Class Taxonomy Mapping & Audit\n",
                "- **04** — Zero-Bias Balancing (~500/class) & Stratified Split (70/15/15)\n",
                "- **05** — Class-Weighted Loss & PyTorch DataLoaders\n",
                "- **06** — Train Fast Model: MobileNetV3-Large (Early Stopping)\n",
                "- **07** — Train Flagship Model: DINOv2 ViT-S/14 (Early Stopping)\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## SECTION 02 — Ingest Open Scrap Datasets\n",
                "Downloads:\n",
                "1. **TrashBox-testandvalid** (Subfolders: cardboard, paper, glass, metal, plastic, e-waste chips, wires, laptops, smartphones, small appliances)\n",
                "2. **TrashBox Main Repo** (Primary train set)\n",
                "3. **TrashNet** (Clean studio recyclables baseline)\n"
            ]
        },
        {
            "cell_type": "code",
            ...
            "source": [
                "# ============================================================\n",
                "# SECTION 02 — Download Datasets (Zero API Keys Needed)\n",
                "# ============================================================\n",
                ...
                "    print('Cloning TrashBox-testandvalid...')\n",
                ...
                "\n",
                "# 2. TrashBox Main Dataset\n",
                "if not os.path.exists(TRASHBOX_MAIN_DIR) or len(os.listdir(TRASHBOX_MAIN_DIR)) == 0:\n",
                "    print('Cloning TrashBox Main Dataset...')\n",
                "    !git clone --quiet --depth 1 https://github.com/nikhilvenkatkumsetty/TrashBox.git {TRASHBOX_MAIN_DIR}\n",
                "    print('TrashBox Main Dataset ready.')\n",
                "else:\n",
                "    print('TrashBox Main already downloaded.')\n",
                "\n",
                "# 3. TrashNet\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "Maps all dataset subfolders to the 16 canonical categories covering E-Waste, Metals, Paper/Cardboard, Plastics, Glass, and Wood."
            ]
        },
        {
            "cell_type": "code",
            ...
            "source": [
                ...
                "all_recs.extend(scan_image_folder(TRASHBOX_MAIN_DIR, 'TrashBox_Main'))\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## SECTION 04 — Zero-Bias Balancing (~500/class) & Stratified Split\n",
                "Downsamples majority classes to 500 images and resamples/augments minority classes to 500 images, ensuring equal representation across all 16 classes."
            ]
        },
        {
            "cell_type": "code",
            ...
            "source": [
                "# ============================================================\n",
                "# SECTION 04 — Balancing & Stratified Split\n",
                "# ============================================================\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "Implements an early stopping callback monitoring validation Macro F1 score to terminate training dynamically."
            ]
        },
        {
            "cell_type": "code",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## SECTION 06 — Train Fast Edge Model: MobileNetV3-Large\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "mb_early = EarlyStopping(patience=5, min_delta=0.002, mode='max')\n",
                "mb_best_path = os.path.join(MODEL_DIR, 'mobilenetv3_best.pt')\n",
                "\n",
                "print('Starting MobileNetV3 training with Early Stopping...')\n",
                ...
                "    print(f'[MobileNetV3] Epoch {epoch:02d}/20 | Loss: {total_loss/total:.4f} | ValAcc: {val_acc:.3f} | ValMacroF1: {val_f1:.4f} {status}')\n",
        ...
        },
        {
            ...
            "metadata": {},
            "source": [
                "## SECTION 07 — Train High-Accuracy Flagship: DINOv2 ViT-S/14\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=20)\n",
                "dino_early = EarlyStopping(patience=5, min_delta=0.002, mode='max')\n",
                "dino_best_path = os.path.join(MODEL_DIR, 'dinov2_vits14_best.pt')\n",
                "\n",
                "print('Starting DINOv2 training with Early Stopping...')\n",
                "for epoch in range(1, 21):\n",
                "    dino_model.train()\n",
                "    total_loss, correct, total = 0.0, 0, 0\n",
                "    for imgs, labels in train_loader:\n",
                ...
                "    print(f'[DINOv2] Epoch {epoch:02d}/20 | Loss: {total_loss/total:.4f} | ValAcc: {val_acc:.3f} | ValMacroF1: {val_f1:.4f} {status}')\n",
...
if __name__ == "__main__":
    ...
    p1 = os.path.join(base_dir, "kabadiwala_colab_master_pipeline.ipynb")
    p2 = os.path.join(base_dir, "sih_kabadiwala_ver1.ipynb")
